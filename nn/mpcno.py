...
class GeoEmbedding(nn.Module):
    ...
    def __init__(self, geo_channels, in_channels, out_channels, geo_act='softsign'):
        ...
        self.wx = nn.Conv1d(in_channels, out_channels, 1, bias=False)
        self.w = nn.Conv1d(out_channels, out_channels, 1, bias=False)
...
def MPCNO_train_multidist(x_train, aux_train, y_train, x_test_list, aux_test_list, y_test_list,  config, model, label_test_list = None, save_model_name="./MPCNO_model", checkpoint_path=None):
    ...
    for ep in range(current_epoch, epochs):
        ...
        test_lp_losses.append(test_lp_dict)
    

        t2 = default_timer()

        print("Epoch : ", ep, " Time: ", round(t2-t1,3), " Rel. Train L" + str(int(loss_p)) + " Loss : ", train_rel_lp, " Rel. Test L" + str(int(loss_p)) + " Loss : ", test_rel_lp_dict, " Test L" + str(int(loss_p)) + " Loss : ", test_lp_dict,  flush=True)
...
def MPCNO_train_parallel(x_train, aux_train, y_train, x_test, aux_test, y_test, config, model, rank=0, local_rank = 0, world_size=1, save_model_name="./MPCNO_parallel_model", checkpoint_path=None):
    ...
    for ep in range(current_epoch, epochs):
        ...
        with torch.no_grad():
            ...
            # synchronize train error
            test_lp_tensor = torch.tensor(test_lp, device=device)
            ...
            dist.all_reduce(test_lp_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(test_count_tensor, op=dist.ReduceOp.SUM)

            test_lp = test_lp_tensor.item() / test_count_tensor.item()
