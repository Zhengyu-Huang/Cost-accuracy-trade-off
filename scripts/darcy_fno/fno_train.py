"""Train one standard Fourier neural operator (FNO) for the Darcy benchmark.

Run from ``scripts/darcy_fno``. Training reads the requested leading samples
and the final 1,000 test samples from ``../../data/darcy`` and writes the model
and normalizer checkpoints below ``models/``. CUDA is used when available.
Example: ``python3 fno_train.py --n_train 4000 --k_max 16 --n_layer 4 --df 64 --downsample 2``.
"""
...
if __name__ == "__main__":
    ...
    save_model_name = f"models/FNO_model_N{n_train}_k{k_max}_nlayer{n_layer}_df{df}_downsample{downsample}"
    ...
    model = setup_model(in_dim=in_dim, out_dim=out_dim, fc_dim=df, k_max=k_max, n_layer=n_layer, grad_layer=False, dxs=[dx1,dx2], dx_scale=10.0, pad_ratio=0.1, incremental=False)
