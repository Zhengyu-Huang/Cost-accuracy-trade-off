    ...
    for k_max_index, k_max in enumerate(k_max_values):
        for n_layer_index, n_layer in enumerate(n_layer_values):
            for df_index, df in enumerate(df_values):
                ...
                model = setup_model(in_dim=in_dim, out_dim=out_dim, fc_dim = df, k_max=k_max, n_layer=n_layer, grad_layer=False, dxs=[dx1,dx2], dx_scale=10.0, pad_ratio=pad_ratio, checkpoint_path=checkpoint_path+".pth")
                ...
                for i in range(n_trial):
                    ...
                    cost[k_max_index, n_layer_index, df_index, i, 0] = mno_floating_point_cost(
                        ...
                        grad_layer=False, pad_ratio=pad_ratio,
                    )
