...
from pcno.geo_utility import convert_structured_data, preprocess_data_mesh
...
def preprocess_data(n_train, n_test, downsample = 1, preprocess_data:bool = True, pcno_data_file = "../../data/darcy/pcno_data.npz"):
    ...
    x_train, y_train = features[0:n_train,...,[0,2,3]], features[0:n_train,...,[1]]
    x_test, y_test = features[-n_test:,...,[0,2,3]], features[-n_test:,...,[1]]
...
def setup_model(in_dim, out_dim, device, checkpoint_path = None):
    nlayers = 5
    ndim = 2
    L_b = 1.1  #at least 2 times larger
