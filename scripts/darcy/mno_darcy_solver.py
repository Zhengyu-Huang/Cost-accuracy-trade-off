"""Benchmark trained MNO checkpoints on Darcy-flow test samples.

Run from ``scripts/darcy``. The default driver requires CUDA, the final 100
raw files in ``../../data/darcy``, and all nine matching checkpoints plus
normalizer states in ``models/``. It writes
``data/cost_accuracy_mno_solver_data.npz`` after CPU and GPU evaluation.
Example: ``python3 mno_darcy_solver.py``.
"""
...
def cost_accuracy_mno_solver_helper(device, downsample, k_max_values, n_layer_values, df_values, n_train, n_trial):
    """Benchmark MNO configurations on held-out Darcy-flow samples.
...
def mno_solver(test_index, downsample):
    """Run the fixed trained MNO configuration and save one prediction."""
...
if __name__ == "__main__":
    ...
    cost, accuracy  = cost_accuracy_mno_solver(downsample_values = [2, 3, 4], k_max_values = [16], n_layer_values = [4, 5, 6], df_values = [64], n_train=4000, n_trial=100)
    
    
    #########################################################################
    # This uses the $N=4000$, $k_{\max}=16$, $d_g=64$, $L=6$ checkpoint and writes `data/mno_solver_data.npz`.
    #########################################################################
        
    # mno_solver(test_index=9999, downsample=2)
